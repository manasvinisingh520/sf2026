...
Examples:
  ...
  python dge.py --group1 "A" --group2 "B" --region "V1"
    ...
    target_cells_per_bin = 100  # Target number of cells per bin
    ...
    if True and n_after > max_cells_for_deseq2:
        ...
        print(f"   Aggregating cells into pseudobulk samples (target: ~{target_cells_per_bin} cells per bin)...")
        ...
        for group_name in [group1, group2]:
            ...
            for patient_name in unique_patients:
                ...
                # Calculate number of bins needed for this patient to get ~target_cells_per_bin cells per bin
                # Use ceiling to ensure we have enough bins (at least 1 bin)
                n_bins_for_patient = max(1, int(np.ceil(n_cells_per_patient / target_cells_per_bin)))
                
                # Calculate base cells per bin (divide evenly)
                base_cells_per_bin = n_cells_per_patient // n_bins_for_patient
                remainder = n_cells_per_patient % n_bins_for_patient
                
                print(f"      Patient {patient_name}: {n_cells_per_patient:,} cells -> {n_bins_for_patient} bin(s) (~{base_cells_per_bin}-{base_cells_per_bin+1} cells/bin)")
                
                # Split patient's cells into bins
                cell_idx = 0
                for bin_idx in range(n_bins_for_patient):
                    # Distribute remainder cells across first few bins
                    ...
                    actual_cells_in_bin = patient_cells_subset.n_obs
    ...
    dge = DGEAnalyzer(
        ...
        topK=10,
        output_file=f"dge_results_{region}_{group1}_vs_{group2}_bins{target_cells_per_bin}.csv",
        min_counts=10,
        n_cpus=2
    )
